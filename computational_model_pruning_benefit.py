...
class Game:
  ...
  def __init__(self, num_spaces, winning_paths, board={}, whos_turn=c.COMPUTER, other_player=c.HUMAN, reduced_opponent=True, noise=c.NOISE_HUMAN, heuristic="paths", exp=1, interaction=False, opponent=False, neighborhood=2, prune = False, potential='square', shutter=False):
    ...
    self.x_missed_wins = 0.0
    self.x_wins_opportunity = 0.0
    self.o_missed_wins = 0.0
    self.o_wins_opportunity = 0.0
  ...
  def play_game(self, max_depth = False):
    ...
    # while not winning_player:
      self.node_count = 0
      # self.display_game()  # TODO: bring back if want to display
  ...
  def minimax_max_alphabeta_DL(self, alpha, beta, board, depth, prev_space_x = None, stochastic_choic=False):
    '''Minimax algorithm with alpha-beta pruning and depth-limited search. '''
    # or ((depth==self.max_depth-1) & (beta==c.LOSE_SCORE))
    if (board.is_terminal()) or (depth <= 0) or (self.node_count >= self.max_moves) or (self.nodes_computed_counter >= self.max_nodes_computed):
  ...
  def minimax_min_alphabeta_DL(self, alpha, beta, board, depth, prev_space_x = None):
    ...
    #   print 'happened'
    if (board.is_terminal()) or (depth <= 0) or (self.node_count >= self.max_moves) or (self.nodes_computed_counter >= self.max_nodes_computed) or  ((depth==self.max_depth-2) & (beta==c.LOSE_SCORE)):
      # return (board.obj(c.WIN_DEPTH-depth), None) # Terminal (the space will be picked up via recursion)
          ...
          for noise in noise_vals:
            for shutter_size in shutter_sizes:
              ...
              for filename in os.listdir("predefinedBoards/"):
                ...
                # print move_matrix
                o_blindness = 0.0
                if game.o_wins_opportunity > 0:
                  o_blindness = game.o_missed_wins/game.o_wins_opportunity
                x_blindness = 0.0
                if game.x_wins_opportunity > 0:
                  x_blindness = game.x_missed_wins/game.x_wins_opportunity
                board_results.append(o_blindness)
                board_results.append(x_blindness)
